...
The agent's `POST /agent/progress` announcement is the authoritative `currentChallengeId` and journals with `via: "self"`. The backend also guesses from a command (`via: "command"`) or an agent prose sentence (`via: "message"`). A guess only fills an empty target or replaces a target that the entrant has solved. It cannot move a live self-reported target, while a new self-report can move any target. Command and prose guesses require one unsolved challenge reference; ambiguous references are ignored. Each accepted change journals `entrant.challenge`, and `evidence` records the matched text. The snapshot stays `null` until a source names a challenge; old events without `via` or `evidence` count as self-reports.
