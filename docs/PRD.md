...
- **fresh wallet per entrant per run**; its private key comes from a verified funder seed signature and lives only in process memory, while SQLite stores the address. the funding gate only watches balances (ADR-0005, amended by ADR-0011 and ADR-0012).
