...
**Trade-off:** the backend gains a second outbound network dependency (OpenRouter's model list, after narration). mitigated: the curated path never touches it, so race day works offline; a non-curated model that cannot be verified is rejected with a clear message rather than accepted blind; the cache serves stale on refetch failure and the search answers 503 when there is nothing cached. floating `~` aliases and `:free` variants are filtered out so a pinned id never changes identity under the arena. labels ride the endpoint because they are facts about a model, not presentation; colours and avatar tags stay in the frontend. `RosterEntry.effort` becoming required is a wire change that only the ai.ctf frontend and the mock frontend feel; both already pin efforts. gpt-5.6-sol returns to the list unrehearsed since #48 saw it refuse the briefing; the briefing has changed since (#76) and the operator asked for it.

**Consequence:** `arena-types.ts` and `API.md` bump; the ai.ctf frontend re-copies the contract per ADR-0002 and rebuilds its picker on `GET /agents`. the mock frontend fetches the same endpoint with the smallest change that keeps the workspace green; it is no longer the reference UI, the ai.ctf repo is. `pricing.ts` rates are refreshed once from the vendor pricing pages (sonnet 5 had dropped) and every codex and claude agent has a row, enforced by the existing test. two follow-ups are recorded, not done: harness availability on the endpoint (whether this deployment holds the credential for a harness) and generating the shared types from the backend's zod schemas through an OpenAPI document, which is the agreed successor to the file copy.
