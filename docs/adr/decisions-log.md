...
**Why:** carlos's objection (2026-07-29 meeting) was to plaintext keys at rest, and the stated reason for storing them — restoring past runs — has no code behind it and resume was ruled out anyway. a signer that uses deterministic nonces (RFC 6979 — what common wallets implement, but a property of the signer, not of ECDSA itself) re-signs the same message to identical bytes, so the funder can re-derive every burner key offline — sweep after a crash, after teardown, after the box is gone. that recovers leftover funds without the arena holding a secret or shipping sweep logic.
...
**Consequence:** the message signed must vary per run (flags can't mint twice to one address, so every run needs virgin wallets — same constraint that drove ADR-0005). recovery of a run's funds is the funder's capability, not the server's. the funder must sign from a plain EOA — a Safe or MPC signer has no deterministic personal signature to re-derive from. open check: confirm the treasury signer austin uses on stream is an EOA *and re-signs a test message to identical bytes* — a randomized-nonce signer produces a different valid signature each time, which verifies fine but re-derives different keys. as a backstop that removes the determinism dependency entirely, the funder saves the canonical signature out-of-band at race time (it is the recovery key for that run's leftovers; treat it like one).

---

## ADR-0014 — funding moves to the funder; the gate only watches

**Status:** accepted (2026-07-30) — with ADR-0013, supersedes the funding half of ADR-0005; from #28 and the 2026-07-29 meeting. superseded in part by ADR-0019 (2026-08-11): the profile-pinned `funderAddress` is gone; the operator allowlist authorizes the seed signature. updated 2026-09-14: the funding timeout is removed on every profile; a run waits in awaiting_funding until the operator funds or stops it.
...
## ADR-0021 — a cheap model narrates each entrant; the line rides the feed as `entrant.narration`

**Status:** accepted (2026-08-20), from ai.ctf issue #46 (Austin's feedback). design in `docs/narration.md`

**Decision:** the backend gains its first outbound model call. a per-run watcher, started and aborted like `SolveWatch`, reads each running entrant's events from the journal and asks a cheap model (default `anthropic/claude-haiku-4.5`, over OpenRouter, through the vercel ai sdk) for one or two short sentences on what the entrant is doing. the text is journaled as a new event type `entrant.narration` `{entrantId, text, basedOnEventId}` under the entrant's own source, and the snapshot exposes the latest as `EntrantSummary.narration`. timing is a floor and a ceiling per entrant: a call when new events exist and 10 s have passed, a call regardless after 90 s of silence while the entrant is `working` or `blocked`, no ceiling while `idle` (the between-turns state, which a steer or nudge ends), one closing line at `done`. the `EntrantStatus` enum does not change.

**Why:** spectators could not tell what any agent was doing — the feed is noise, the console is a wall of text, and `currentChallengeId` names the target but not the activity. ADR-0011 refused to widen the status enum because the backend had no honest source for "stuck" or "exploiting"; a model-written line is that richer vocabulary, and it belongs on the feed as an event, labelled as a guess, exactly where ADR-0011 said it should go. the backend owns it because only the backend has the full transcript (the wire caps strings at 4000 chars, the frontend keeps 70 console rows) and one call then serves every viewer. the ceiling exists because the opencode parser emits tool events only on completion, so a lane inside a long command would otherwise go silent.
...
## ADR-0023 — the backend serves the agent list; the contract file carries vocabulary, not data

**Status:** accepted (2026-09-08), from ai.ctf issue #61 (agent picker), itself from the #60 discussion

**Decision:** the set of agents the arena can run — harness, model, the effort levels that model accepts, plus a label and a vendor — lives in one backend module and is served on `GET /agents`. `GET /agents/search?harness=&q=` answers a typeahead from the same data; for the opencode harness it also returns any OpenRouter model that supports reasoning, read from OpenRouter's public model list (no key) and cached in the backend for ten minutes. `POST /runs` validates every roster entry against the same registry: a curated agent, or for opencode a model verified on OpenRouter, and an effort from that agent's list. effort is mandatory on a roster entry. `ROSTER_MODELS` and `OPENCODE_EFFORTS` leave `arena-types.ts`; the file keeps the closed vocabularies (`HARNESS_IDS`, `ROSTER_EFFORTS`, which gains `ultra`) and the response types. presets keep their entrants but the default race lineup is a frontend constant, and the picker edits that lineup rather than composing one from nothing. sonnet 4.5 and haiku 4.5 are not listed because claude code has no effort setting for them, which keeps "every agent has efforts" true with no special case.

**Why:** the frontend hardcoded ten entrants and the backend hardcoded an allowlist, and the two drifted (the docker preset had three entrants, the frontend ten; the backend accepted `max` on gpt-5.5, which codex does not support). models change every week — see the roster commits — while the contract's shapes change rarely, so the model list is data, not contract, and belongs behind an endpoint. effort became mandatory because a race that depends on a vendor default is not reproducible, and the codex and claude CLIs both silently clamp or default rather than error. per-model effort sets exist because the CLIs differ per model (no `xhigh` on opus 4.6, no `max` on gpt-5.5, `ultra` only on the gpt-5.6 pair). OpenRouter search is opencode-only because OpenRouter publishes a model list and codex and claude do not; a picker that lets an operator start a lane that dies at prepare is worse than a curated list. "agent" is the chosen word: an agent is what can run, an entrant is an agent placed in a run.

**Trade-off:** the backend gains a second outbound network dependency (OpenRouter's model list, after narration). mitigated: the curated path never touches it, so race day works offline; a non-curated model that cannot be verified is rejected with a clear message rather than accepted blind; the cache serves stale on refetch failure and the search answers 503 when there is nothing cached. floating `~` aliases and `:free` variants are filtered out so a pinned id never changes identity under the arena. the search lists any model that advertises `reasoning`, not only those that also advertise `reasoning_effort`: OpenRouter converts an effort level into a share of the model's thinking budget for models that only take a budget, so an effort never kills a turn; rehearsed 2026-09-09 with minimax-m2.7 (budget-only) at `high`, which reasoned on every turn. requiring `reasoning_effort` would have halved the searchable pool and dropped five curated models. labels ride the endpoint because they are facts about a model, not presentation; colours and avatar tags stay in the frontend. `RosterEntry.effort` becoming required is a wire change that only the ai.ctf frontend and the mock frontend feel; both already pin efforts. gpt-5.6-sol returns to the list unrehearsed since #48 saw it refuse the briefing; the briefing has changed since (#76) and the operator asked for it.

**Consequence:** `arena-types.ts` and `API.md` bump; the ai.ctf frontend re-copies the contract per ADR-0002 and rebuilds its picker on `GET /agents`. the mock frontend fetches the same endpoint with the smallest change that keeps the workspace green; it is no longer the reference UI, the ai.ctf repo is. `pricing.ts` rates are refreshed once from the vendor pricing pages (sonnet 5 had dropped) and every codex and claude agent has a row, enforced by the existing test. the docker presets now pin `high` effort instead of inheriting the vendor default, so preset races cost the same and take as long as a `high` roster race; the opencode preset entrant now receives effort through `OPENCODE_CONFIG_CONTENT` in the container environment, like any roster entrant with an effort. this replaces a file in the working directory so the entrant cannot edit its effort config. two follow-ups are recorded, not done: harness availability on the endpoint (whether this deployment holds the credential for a harness) and generating the shared types from the backend's zod schemas through an OpenAPI document, which is the agreed successor to the file copy.

## ADR-0024 — external entrants attach over a plain HTTPS agent API; the substrate switch moves from the run to the entrant

**Status:** accepted (2026-09-08), from ai.ctf issue #60 ("connect outsiders / bring your own agent"). design in `docs/external-entrants.md`; research in `docs/research/byoa-*.md`. updated by ADR-0025 (2026-09-09) for credentials, reporting, and status before any of this merged: the per-entrant token, the six event types, the hook route, and the idle timer below did not ship. updated 2026-09-15: a wallet that already holds any flag is rejected at first entry (409) instead of entering with a displayed pre-held count; the count field is gone. Reason: the poller scores lifetime mints, so a pre-held flag would count as a solve, and a head start the board cannot verify has no place on it. updated 2026-09-15 (evening): the zero-flags rule is enforced again when the run starts, removing any outside lane that gained flags in the lobby, with the reason on the board; the poller records the run's start block and never scores a mint from before it.

**Decision:** an entrant gains a `kind`: `hosted` (the arena runs it, today's entrant, unchanged) or `external` (someone else runs it on their own machine with their own key and gas). an external entrant registers itself against a run — from creation until stop — by signing a plain EIP-191 text (`Join Agents Arena run {runId} as {address} with nonce {nonce}`) with its agent wallet and posting it with a display name and optional self-declared `harness`/`model`/`effort`/`url`. it gets a per-entrant bearer token, stored hashed in SQLite, and four agent routes: `GET /agent/task` (the briefing, null until `running`), the existing `POST /agent/progress`, `POST /agent/events` (batched, six event types in the arena's own vocabulary, client `seq` for idempotent retries), and `GET /agent/inbox` (steers and broadcasts, cursor-paged, fetching is delivery). the wallet is the identity: one wallet, one entrant per run, server-assigned id `ext-` plus twelve hex characters of the address; rejoining replaces the entry and revokes the old token. the operator can remove an external entrant at any time. the funding gate, ready barrier, seed derivation, sweep, restart, and preflight skip external entrants. behind the seams, the docker-versus-fake choice becomes per entrant: an `ExternalDriver` implements the existing five-method `EntrantDriver` (prepare no-op, start journals the task as `entrant.prompt`, steer and broadcast enqueue to the inbox, restart refuses, stop revokes the token), and one task function produces the briefing for hosted prompt and external task alike. two new event types, `entrant.joined` and `entrant.removed`, let a board follow the roster mid-race. status for an external lane is derived from its reports (any event marks `working`, two quiet minutes mark `idle`, stop or remove marks `done`), with an explicit `entrant.status` event overriding.

**Why:** the arena's value is the shared board, and the board already keys everything on a wallet address — the solve poller, the leaderboard, the scores table. an outsider who mints flags from their own wallet is already scored by the chain; what was missing was a lane and a way in. the protocol survey (`docs/research/byoa-agent-protocols.md`) found that every published protocol either points the connection the wrong way for a laptop behind NAT (AG-UI, A2A, the arena calling in), or has no network transport at all (ACP is stdio). plain HTTPS with the agent dialing out is the only shape that works on a livestream deadline with a stranger's machine. the harness survey (`docs/research/byoa-harness-hooks.md`) showed that every harness we care about can post to an HTTP endpoint from a hook or plugin with a token from an environment variable, so a batched events route in our own vocabulary is enough for a live lane without a connector process. the wallet-signed join means no accounts, no operator approval step, and `cast wallet sign` is all an outsider needs. a per-entrant driver instead of a per-run substrate keeps `RunManager` ignorant of kinds: it still calls prepare/start/steer/restart/stop on whatever driver the entrant resolves to.
...
## ADR-0026 — an arena token earned in two tool calls replaces the wallet token

**Status:** accepted (2026-09-13). updates ADR-0025 for credentials; the tools, reporting, status, and protocol-revision decisions there stand. design in `docs/external-entrants.md`; wire contract in `contract/API.md`.

**Decision:** on Base, the chain the race runs on, the CTF contract mints each flag once per wallet, so a wallet that raced once cannot solve the challenges again. one wallet per run is a fact of the game there, not a rule the arena chose, and a credential that outlives the run is never used again. a local tester who redeploys the contracts proves the same wallet again for the new run. the wallet-scoped agent token, `POST /agent/register`, the register page, and the `agent_tokens` table go. in their place the agent earns an arena token itself, with two tool calls and its own wallet.

the harness config holds no credential: the person adds the arena to their harness with the URL alone. two tools replace `join_run`. `request_nonce` takes an address and returns the sentence to sign, `Enter Agents Arena as {address} with nonce {nonce}`; the nonce is the existing single-use, ten-minute one; the server stores nothing when it issues one and checks it by its own signature, as it does for operator login. the agent signs the sentence with its wallet, however the person gave it one. `enter_run` takes the name, address, nonce, and signature, plus the optional run id, harness, model, effort, and url; it rebuilds the sentence from the address and nonce, checks the signer against it, consumes the nonce, creates or rejoins the lane, and returns the lane id and the arena token. run selection is unchanged from ADR-0025: an omitted run id selects the wallet's live run or the only open run.

an arena token is good for one lane in one run. the external lane's row stores its hash. it dies when the run ends, when the operator removes the lane, or when a fresh `enter_run` replaces it. rejoining with a fresh signature keeps the lane and its history and issues a new arena token; that is how an agent whose context reset gets back in. removed wallets still cannot rejoin that run.

the arena token travels where each door can carry it. on MCP it is a required `token` argument on every other tool (`get_task`, `set_current_challenge`, `post_note`, `read_inbox`), because a harness sets headers once at config time and the model cannot change them. on the HTTP door it is the bearer header, as before: `GET /auth/nonce` is the proving step, `POST /agent/enter` takes the address, signature, and declared fields and issues the arena token, and the four lane routes read `Authorization: Bearer`. one resolver accepts either place. `request_nonce` and `enter_run` need no credential; the other four refuse a missing, wrong, or dead arena token with one fixed sentence as a tool error, never HTTP 401 (the harness OAuth-discovery reason from ADR-0025 holds). rate limits key on the arena token hash, one record per arena token, as they keyed on the token before. the tool list is public and identical for every caller: six tools, in this order, `request_nonce`, `enter_run`, `get_task`, `set_current_challenge`, `post_note`, `read_inbox`.

**Why:** the ninety-day, then one-year, token was built for reuse across races that the contract forbids. the register page's "skip if you already have one" and "good for every race after" were false: every race on Base needs a fresh wallet, so every race needs the proving step anyway, and a separate register step plus a token pasted into four harness configs was an extra setup step that bought nothing. moving the proof into the run makes the harness config a URL, which is the shortest setup an MCP server can have. the signature is one line for the agent, and it is what stops anyone who knows the URL from entering as a hosted agent's address and showing its flags on their own lane. two tools rather than one because the agent has to see the sentence before it can sign it; the model signs between them with whatever wallet tooling it has.

**Trade-off:** the arena token lives in the model's context and in tool inputs. ADR-0025 rejected that for a credential worth a year of races; an arena token is worth one lane in one run. whoever reads an arena token can post notes and set the current challenge on that lane until the run ends or the agent rejoins; they cannot mint flags, and the flag count stays the only truth. the journal already scrubs token shapes from text and must keep covering the arena token shape, and nothing may journal MCP tool inputs for an external lane (today nothing does). the arena token is one more argument on every tool call. a context reset costs the agent a fresh signature with the same wallet, not a page visit; the person is told to race one wallet and to come back with it. `request_nonce` is open, so anyone can ask for nonces; a nonce is cheap, lasts ten minutes, and proves nothing on its own.

rejected choices:

- operator-issued invite links per run: nobody proves wallet control, so anyone with a link could enter as any address, including a hosted agent's, and show its flags on their own lane. the signature is one line and prevents it.
- a per-run token minted by a register step: the same work for the racer, still a config paste, still a register page for a step that happens once per wallet anyway.
- a signature on every call: proves the wallet each time but too heavy for the model; the arena token already binds the call to a lane the wallet proved.
- keeping the wallet token and counting only solves after join: dead, because the contract cannot mint a flag twice to one wallet, so a returning wallet has nothing to solve.
- one entry tool with a time the agent writes into the sentence instead of a server nonce: the same safety, since the server would accept a signature dated within ten minutes and refuse one it had seen, but the model would have to compose the sentence byte for byte. weighed and rejected 2026-09-13; `request_nonce` hands the model the exact bytes to sign, and the extra call costs it nothing.
- the arena token in a harness header: the model cannot change harness headers after config, so a arena token scoped to one run would need a config edit per run, which is the work this ADR removes.

**Consequence:** `agent_tokens` and `POST /agent/register` go, with no migration, so an old local database file keeps the empty table; `external_entrants` gains an arena token hash column with a fresh name (a `token_hash` column and its migration shim were removed from this branch on 2026-09-13 as first-version leftovers and are not revived). `contract/arena-types.ts` replaces `REGISTER_MESSAGE_TEMPLATE` with the enter sentence and lists six tools in `AGENT_MCP_TOOLS`; `API.md` bumps and the ai.ctf frontend re-copies per ADR-0002. the MCP input schemas for the four lane tools gain `token`. the arena's words change: "register" leaves the glossary, "request nonce" and "arena token" enter, and "enter" replaces "join" because the run now hands out an arena token; `token` is the field and "arena token" the prose. the frontend's register page becomes a setup page (create the wallet, then the URL in the harness) and its join page keeps the two checks and the sentence; its no-MCP recipe becomes nonce, sign, enter, then bearer. hosted entrants keep their container tokens, unchanged.

**Future identities:**

the agent owns its identity. today its wallet is that identity because the chain judges the race. only the proving step assumes a wallet: `request_nonce` hands out a sentence and `enter_run` checks its signer; the entrant id comes from the address; the live-lane lookup joins `external_entrants` on address; and the scorer reads flags by address. everything after the arena token, meaning the tools, inbox, status, and journal, never sees an address. a game outside the chain swaps the proving step and nothing else: a person logs in with OAuth, a login authorization protocol, on a page and gets the same kind of arena token for a lane in a run. the entrant id comes from that identity key, and a different judge replaces the chain. nothing after the arena token changes.

**Amended 2026-09-13:** the credential is called the arena token, field `token`; "run pass" lasted one afternoon.

**Amended 2026-09-13:** the first entry tool is `request_nonce`, renamed from `prove_wallet`.
