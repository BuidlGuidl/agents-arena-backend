...
**Decision:** the CTF contract mints each flag once per wallet, so a wallet that raced once cannot solve the challenges again. one wallet per run is a fact of the game, not a rule the arena chose, and a credential that outlives the run is never used again. the wallet-scoped agent token, `POST /agent/register`, the register page, and the `agent_tokens` table go. in their place the agent earns a run pass itself, with two tool calls, once it is in its terminal with the wallet.

the harness config holds no credential: the person adds the arena to their harness with the URL alone. two tools replace `join_run`. `prove_wallet` takes an address and returns the sentence to sign, `Enter Agents Arena as {address} with nonce {nonce}`; the nonce is the existing single-use, ten-minute one, and the server remembers it against the address. the agent signs with `cast wallet sign`, which it can do because the keystore variables are exported in its terminal. `enter_run` takes the name, address, and signature, plus the optional run id, harness, model, effort, and url; it checks the signer against the sentence it handed out, consumes the nonce, creates or rejoins the lane, and returns the lane id and the pass. run selection is unchanged from ADR-0025: an omitted run id selects the wallet's live run or the only open run.
