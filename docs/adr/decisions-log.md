...
`GET /auth/nonce` is open and every call inserts into an in-memory map, so a flood grows it until the 10-minute TTL catches up. a plain size cap is the wrong fix: evicting oldest-first deletes the nonce the operator is holding while his wallet prompt is open, which turns a memory drip into a login lockout during the event. the answer is rate limiting at whatever fronts the backend, or nonces that carry their own signed expiry and need no storage at all. left as it is for now, deliberately.
