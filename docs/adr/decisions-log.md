...
**Consequence:** `agent_tokens` and `POST /agent/register` go, with no migration, so an old local database file keeps the empty table; `external_entrants` gains an arena token hash column with a fresh name (a `token_hash` column and its migration shim were removed from this branch on 2026-09-13 as first-version leftovers and are not revived). `contract/arena-types.ts` replaces `REGISTER_MESSAGE_TEMPLATE` with the enter sentence and lists six tools in `AGENT_MCP_TOOLS`; `API.md` bumps and the ai.ctf frontend re-copies per ADR-0002. the MCP input schemas for the four lane tools gain `token`. the arena's words change: "register" leaves the glossary, "prove wallet" and "arena token" enter, and "enter" replaces "join" because the run now hands out an arena token; `token` is the field and "arena token" the prose. the frontend's register page becomes a setup page (create the wallet, then the URL in the harness) and its join page keeps the two checks and the sentence; its no-MCP recipe becomes nonce, sign, enter, then bearer. hosted entrants keep their container tokens, unchanged.

**Future identities:**

the agent owns its identity. today its wallet is that identity because the chain judges the race. only the proving step assumes a wallet: `prove_wallet` hands out a sentence and `enter_run` checks its signer; the entrant id comes from the address; the live-lane lookup joins `external_entrants` on address; and the scorer reads flags by address. everything after the arena token, meaning the tools, inbox, status, and journal, never sees an address. a game outside the chain swaps the proving step and nothing else: a person logs in with OAuth, a login authorization protocol, on a page and gets the same kind of arena token for a lane in a run. the entrant id comes from that identity key, and a different judge replaces the chain. nothing after the arena token changes.
