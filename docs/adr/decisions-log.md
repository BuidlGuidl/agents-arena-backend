...
## ADR-0026 — a run pass earned in two tool calls replaces the wallet token
...
**Trade-off:** the pass lives in the model's context and in tool inputs. ADR-0025 rejected that for a credential worth a year of races; a pass is worth one lane in one run. whoever reads a pass can post notes and set the current challenge on that lane until the run ends or the agent rejoins; they cannot mint flags, and the flag count stays the only truth. the journal already scrubs token shapes from text and must keep covering the pass shape, and nothing may journal MCP tool inputs for an external lane (today nothing does). the pass is one more argument on every tool call. a context reset costs the agent a fresh signature with the same wallet, not a page visit; the person is told to race one wallet and to come back with it. `prove_wallet` is open, so anyone can ask for nonces; a nonce is cheap, lasts ten minutes, and proves nothing on its own.
...
- a signature on every call: proves the wallet each time but too heavy for the model; the pass already binds the call to a lane the wallet proved.
...
- the pass in a harness header: the model cannot change harness headers after config, so a per-run pass would need a config edit per run, which is the work this ADR removes.

**Consequence:** `agent_tokens` and `POST /agent/register` go, with no migration, so an old local database file keeps the empty table; `external_entrants` gains a pass hash column with a fresh name (a `token_hash` column and its migration shim were removed from this branch on 2026-09-13 as first-version leftovers and are not revived). `contract/arena-types.ts` replaces `REGISTER_MESSAGE_TEMPLATE` with the enter sentence and lists six tools in `AGENT_MCP_TOOLS`; `API.md` bumps and the ai.ctf frontend re-copies per ADR-0002. the MCP input schemas for the four lane tools gain `pass`. the arena's words change: "register" leaves the glossary, "prove wallet" and "run pass" enter, and "enter" replaces "join" because the run now hands out a pass; `pass` is the field and "run pass" the prose. the frontend's register page becomes a setup page (create the wallet, then the URL in the harness) and its join page keeps the two checks and the sentence; its no-MCP recipe becomes nonce, sign, enter, then bearer. hosted entrants keep their container tokens, unchanged.

**Future identities:**

the agent owns its identity. today its wallet is that identity because the chain judges the race. only the proving step assumes a wallet: `prove_wallet` hands out a sentence and `enter_run` checks its signer; the entrant id comes from the address; the live-lane lookup joins `external_entrants` on address; and the scorer reads flags by address. everything after the pass, meaning the tools, inbox, status, and journal, never sees an address. a game outside the chain swaps the proving step and nothing else: a person logs in with OAuth, a login authorization protocol, on a page and gets the same kind of pass for a lane in a run. the entrant id comes from that identity key, and a different judge replaces the chain. nothing after the pass changes.
