...
task function shared by prompt builder and `GET /agent/task`; `POST /agent/events` with zod, limits, seq dedupe, rate limit, redaction, challenge heuristics on `tool.call` and `agent.message`, journal under the entrant's source; derived status with idle timer and explicit override; `GET /agent/inbox` with cursor, delivery journaling, poll rate limit; steer/broadcast enqueue; narration for late joiners; `agentCount` and any snapshot totals include externals.
