...
- **funding gate** — the `awaiting_funding` hold where the run waits for each entrant's balance to cross the profile's floor. the gate only watches; it never sends. the funder pays the displayed addresses himself (Austin on stream, possibly via a multisend); on local a dev-only helper auto-funds from the anvil account so the loop stays one-click (ADR-0014).
- **seed signature** — the funder's EIP-712 signature over per-run typed data (domain `agents-arena` + chainId, type `Seed {runId}`), made with the wallet that funds the race. the single secret every entrant key derives from (ADR-0013). lives in process memory and in the funder's ability to re-sign; never at rest, never journaled.
