...
- **preset** — the server-side default for a run: its substrate plus a default set of entrants. the frontend sends a preset name; when it also sends a roster, the preset only supplies the substrate. (redefined 2026-09-08; before the roster field existed the preset was the whole run config.)
- **agent** — a harness plus a model the arena can run, with the effort levels that model accepts. the backend owns the list and serves it; the frontend never hardcodes one. a curated agent is on that list; a custom agent is an OpenCode entrant on any OpenRouter model that supports reasoning, verified against OpenRouter's public model list when the run is created. not an entrant: an agent has no id, wallet, or run.
- **roster** — the list of entrants for one run, sent on `POST /runs`. each entry names an agent and pins an effort; the pin is mandatory so a race never depends on a vendor default. the word means only this request field.
- **effort** — the reasoning level pinned on an entrant, from the closed vocabulary low, medium, high, xhigh, max, ultra. each agent accepts a subset; the harness passes it through and the arena never guesses one.
...
- **entrant** — one competitor in a run: an agent (harness + pinned model + pinned effort) given an id, a funded wallet, an erc-8004 identity, and a private credential home, running in its own container. the compared unit is the harness together with its model and effort, not the model alone.
...
- **harness adapter** — the seam that hides harness-specific details. owns the CLI command, credential home, preflight, raw-event parser, and mapping into `ArenaEvent`. the run lifecycle only knows: prepare, report ready, start, steer, restart, stop, emit events.
...
- **subagent call** — a tool call an entrant's delegate made on its behalf (claude's Task). journaled in the entrant's own lane, in arrival order, carrying `parentToolCallId` — the tool call id of the outer call that spawned it. a client badges or nests on that field; the reference frontend renders it flat with a `↳ subagent` marker. the delegate's prose never reaches the lane, only its tool work (ADR-0018).
...
- **funding gate** — the `awaiting_funding` hold where the run waits for each entrant's balance to cross the profile's floor. the gate only watches; it never sends. an operator pays the displayed addresses (Austin on stream, possibly via a multisend); on local a dev-only helper auto-funds from the anvil account so the loop stays one-click (ADR-0014).
- **seed signer** — any operator address in `ARENA_OPERATOR_ADDRESSES`. the signer must be a plain EOA with deterministic signing and holds the run's recovery capability. by convention, whoever funds signs (ADR-0019).
- **seed signature** — the seed signer's EIP-712 signature over per-run typed data (domain `agents-arena` + chainId, type `Seed {runId}`). the single secret every entrant key derives from (ADR-0013). lives in process memory and in the signer's ability to re-sign; never at rest, never journaled.
- **derived burner** — an entrant's wallet whose key is a pure function of the seed signature and the entrant id. the arena stores only its address; the seed signer can re-derive the key offline any time by re-signing.
