...
- **funding gate** — the `awaiting_funding` hold where the run waits for each entrant's balance to cross the profile's floor. the gate only watches; it never sends. the funder pays the displayed addresses himself (Austin on stream, possibly via a multisend); on local a dev-only helper auto-funds from the anvil account so the loop stays one-click (ADR-0012).
- **seed signature** — the funder's signature over a per-run message, made with the wallet that funds the race. the single secret every entrant key derives from (ADR-0011). lives in process memory and in the funder's ability to re-sign; never at rest, never journaled.
