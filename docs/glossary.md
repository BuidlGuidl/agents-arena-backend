...
- **seed signature** — the funder's EIP-712 signature over per-run typed data (domain `agents-arena` + chainId, type `Seed {runId}`), made with the wallet that funds the race. the single secret every entrant key derives from (ADR-0011). lives in process memory and in the funder's ability to re-sign; never at rest, never journaled.
