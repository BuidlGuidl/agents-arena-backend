...
// Claude Code adds fields between versions; API.md promises unknown fields are ignored.
...
// Hook events bypass dedupe, so seq stays a constant.
export function claudeHookToAgentEvents(payload: ClaudeHookPayload): AgentEventInput[] {
  ...
  switch (payload.hook_event_name) {
    ...
    case 'PostToolUse':
      return [{ ...tool, type: 'tool.result', ok: true, detail: responseDetail(payload.tool_response) }];
    case 'PostToolUseFailure':
      return [{ ...tool, type: 'tool.result', ok: false, detail: payload.error ?? responseDetail(payload.tool_response) }];
    case 'Stop':
...
function responseDetail(response: ClaudeHookPayload['tool_response']): string {
