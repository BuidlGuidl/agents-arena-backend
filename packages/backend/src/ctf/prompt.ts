...
export function buildOpeningPrompt(entrant: EntrantRecord, profile: ChainProfile): string {
  ...
  return [
    ...
    '- When you start working on a challenge, and whenever you switch, report it: curl -fsS -X POST "$ARENA_API_URL/agent/progress" -H "authorization: Bearer $ARENA_AGENT_TOKEN" -H "content-type: application/json" -d \'{"challengeId": N}\' with N replaced by the challenge number.',
  ].join('\n');
}
