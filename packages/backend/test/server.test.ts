...
describe('seed endpoint', () => {
  it('auto-signs with an empty operator allowlist and records the dev signer', async () => {
    await withAutoSignEnabled(async () => {
      ...
      const external = await server.manager.join({ runId: created.id, address: LOCAL_DEV_OPERATOR.address, name: 'External', arenaTokenHash: 'test-arena-token-hash', claim: () => {} });
...
describe('sweep endpoint', () => {
  ...
  async function sweepSetup(options: {
    ...
    const { run } = await server.manager.create({ preset: 'fake-duel' });
    await server.manager.join({ runId: run.id, address: LOCAL_DEV_OPERATOR.address, name: 'External', arenaTokenHash: 'test-arena-token-hash', claim: () => {} });
